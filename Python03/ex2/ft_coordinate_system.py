...
def check_coordinates() -> tuple[float, float, float]:
    ...
    z: list[str] = []
    rest = []
    while i < len(coordinates) and not coordinates[i].isdigit():
        i = i + 1
    ...
    while i < len(coordinates):
        ...
        if (coordinates[i] == ' ' or coordinates[i] == ','):
            ...
            while (i < len(coordinates)
                   and coordinates[i] != ' ' and coordinates[i] != ','):
    ...
    if len(rest) != 0:
        ...
        try:
            ...
            x_str = ''.join(x)
            x_float = float(x_str)
            final_coordinates.append(x_float)
            y_str = ''.join(y)
            y_float = float(y_str)
            final_coordinates.append(y_float)
            z_str = ''.join(z)
            z_float = float(z_str)
            final_coordinates.append(z_float)
...
def calculate_distance(
    first_tuple: tuple[float, float, float],
    second_tuple: tuple[float, float, float]
) -> float:
    distance = int(math.sqrt(
        (second_tuple[0] - first_tuple[0])**2
        + (second_tuple[1] - first_tuple[1])**2
        + (second_tuple[2] - first_tuple[2])**2
    ))
    return distance
...
def main() -> None:
    ...
    print(f"It includes: X="
          f"{first_tuple[0]}, Y={first_tuple[1]}, Z={first_tuple[2]}")
    distance: float
    distance = calculate_distance(first_tuple, (0.0, 0.0, 0.0))
